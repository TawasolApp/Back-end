...
const mockProfile = {
  ...
  industry: 'Technology',
  skills: [
    { skill_name: 'JavaScript', position: 'Microsoft', endorsements: [] },
  ],
  profile_picture: '',
  cover_photo: '',
  resume: '',
...
describe('ProfilesService', () => {
  ...
  describe('createProfile', () => {
    ...
    it('should throw BadRequestException if ObjectId is invalid', async () => {
      ...
      const dto: CreateProfileDto = {};
      await expect(
        service.createProfile(invalidId as any, dto),
  ...
  describe('getProfile', () => {
    ...
    it('should throw NotFoundException if profile is not found', async () => {
      ...
      await expect(
        service.getProfile(new Types.ObjectId(), mockProfile._id.toString()),
    ...
    it('should throw BadRequestException if ObjectId is invalid', async () => {
      ...
      await expect(
        service.getProfile(invalidId as any, mockProfile._id.toString()),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('updateProfile', () => {
    ...
    it('should throw BadRequestException if ObjectId is invalid', async () => {
      ...
      await expect(
        service.updateProfile({ firstName: '' }, invalidId as any),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('deleteProfileField', () => {
    it('should remove a profile field and return updated profile', async () => {
      ...
      jest.spyOn(profileModel, 'findOneAndUpdate').mockReturnValue({
        exec: jest
          .fn()
          .mockResolvedValue({ ...mockProfile, headline: undefined }),
      } as any);

      const result = await service.deleteProfileField(
        mockProfile._id,
        'headline',
      );
    ...
    it('should throw BadRequestException if ObjectId is invalid', async () => {
      ...
      await expect(
        service.deleteProfileField(invalidId as any, 'headline'),
      ).rejects.toThrow(BadRequestException);
    });
    it('should throw NotFoundException when updated profile is not found', async () => {
      ...
      jest.spyOn(profileModel, 'findById').mockReturnValueOnce({
        exec: jest
          .fn()
          .mockResolvedValue({ _id: id, [field]: 'Software Engineer' }),
      } as any);
  ...
  describe('addSkill', () => {
    it('should add a skill to profile', async () => {
      ...
      jest.spyOn(profileModel, 'findOneAndUpdate').mockResolvedValue({
        ...
        skills: [
          ...mockProfile.skills,
          { skill_name: 'NestJS', endorsements: [] },
        ],
      });
    ...
    it('should throw NotFoundException if profile is not found', async () => {
      ...
      await expect(
        service.addSkill({ skillName: 'NestJS' }, new Types.ObjectId()),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException if ObjectId is invalid', async () => {
      ...
      await expect(
        service.addSkill({ skillName: 'JavaScript' }, invalidId as any),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe('deleteSkill', () => {
    ...
    it('should throw NotFoundException if skill is not found', async () => {
      ...
      await expect(
        service.deleteSkill('Python', mockProfile._id),
      ).rejects.toThrow(NotFoundException);
    });
    it('should throw BadRequestException if ObjectId is invalid', async () => {
      const invalidId = '12345'; // Not a valid MongoDB ObjectId
      ...
      await expect(
        service.deleteSkill('Python', invalidId as any),
      ).rejects.toThrow(BadRequestException);
    });
  });
    ...
    it('should call deleteProfileField with "headline"', async () => {
      const deleteProfileFieldSpy = jest
        ...
        .mockResolvedValue({
          ...
          headline: undefined,
          skills: mockProfile.skills.map((skill) => ({
            skillName: skill.skill_name,
            endorsements: skill.endorsements,
          })),
          education: mockProfile.education.map((education) => ({
            _id: mockProfile._id,
            school: education.school,
            degree: education.degree,
            field: education.field,
            startDate: education.start_date.toISOString(),
            endDate: education.end_date.toISOString(),
            grade: education.grade,
            description: education.description,
          })),
          certification: mockProfile.certification.map((certification) => ({
            _id: mockProfile._id,
            name: certification.name,
            company: certification.company,
            issueDate: certification.issue_date.toISOString(),
            expiryDate: certification.expiry_date.toISOString(),
          })),
        });

      await service.deleteHeadline(mockProfile._id);
      expect(deleteProfileFieldSpy).toHaveBeenCalledWith(
        mockProfile._id,
        'headline',
      );
    });
  ...
  describe('editEducation', () => {
    it('should edit an education entry successfully', async () => {
      ...
      jest.spyOn(profileModel, 'findOneAndUpdate').mockResolvedValue({
        ...mockProfile,
        education: [{ ...mockProfile.education[0], degree: 'Updated Degree' }],
      });
  ...
  describe('deleteEducation', () => {
    ...
    it('should throw BadRequestException if profile ID is invalid', async () => {
      await expect(
        service.deleteEducation(educationId, '12345' as any),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw NotFoundException if profile is not found', async () => {
      ...
      await expect(
        service.deleteEducation(educationId, profileId),
      ).rejects.toThrow(NotFoundException);
    });
  ...
  describe('addCertification', () => {
    ...
    it('should throw BadRequestException if profile ID is invalid', async () => {
      await expect(
        service.addCertification(certification, '12345' as any),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw NotFoundException if profile is not found', async () => {
      ...
      await expect(
        service.addCertification(certification, profileId),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw NotFoundException when updated profile is not found after adding certification', async () => {
      jest
        ...
        .mockResolvedValue({ _id: profileId, certification: [] });
      jest.spyOn(profileModel, 'findOneAndUpdate').mockResolvedValue(null);
  ...
  describe('editCertification', () => {
    ...
    it('should edit a certification successfully', async () => {
      ...
      const result = await service.editCertification(
        updatedCertification,
        profileId,
        certificationId,
      );
      expect((result.certification ?? [])[0]?.name).toBe(
  ...
  describe('deleteCertification', () => {
    ...
    it('should delete a certification successfully', async () => {
      const mockProfile = {
        _id: profileId,
        certification: [
          { _id: certificationId, name: 'AWS Certified Developer' },
        ],
      };
      ...
      const result = await service.deleteCertification(
        certificationId,
        profileId,
      );
    ...
    it('should throw BadRequestException if profile ID is invalid', async () => {
      await expect(
        service.deleteCertification(certificationId, '12345' as any),
      ).rejects.toThrow(BadRequestException);
    });
